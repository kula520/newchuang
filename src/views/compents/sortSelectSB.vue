  ...
  watch: {
    ...
    ObjDatas: {
      ...
      handler (newName, oldName) {
        ...
        let equipObjSheach = capitalSeachDatas.equipObj
        if (equipObjSheach != '' && equipObjSheach != undefined) {
          that.dataVlas(equipObjSheach)
        }
        // 添加資產
        let capitalDatas = that.$store.state.capitalDatas
        let equipObjCaptal = capitalDatas.equipObj
        if (equipObjCaptal != '' && equipObjCaptal != undefined) {
          that.dataVlas(equipObjCaptal)
        }
      },
      immediate: true,
      deep: true
